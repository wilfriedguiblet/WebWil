...
Monika Michalovova receives the CBIOS predoctoral training grant from the NIH's National Institute of General Medical Sciences !
thumbs-up
...
SLIDESHOW_FIELD_COUNT = 2

SLIDESHOW = """

img/age1.png
We demonstrated the dependence of the total number of heteroplasmies in mothers on their age at collection and in children on maternal age at fertilization (Redolledo-Jaramillo et al. PNAS 2014)

img/boris.png
Boris Rebolledo-Jaramillo presenting at the Biology of Genomes meeting at Cold Spring Harbor. May 2015. Drwaing by Alex Cagan.

img/fin_monkey.png
Primate genome comparison assisted us in elucidating the mechanisms of Transposable Element mutagenesis (Kvikstad & Makova 2010). Credit: Jeff Sorley.

img/fin_front.png
In a recent PNAS article, we segmented the human genome based on divergence rates for four mutation types (Kuruppumullage Don, Ananda, et al. 2013). Credit: Asha Sharath and Guruprasad Ananda

img/fin_horse.png
Przewalski's horse. We sequenced the complete mitochondrial genome of this endangered species (Goto et al. 2011). Credit: Kateryna Makova.

img/fin_fragile.png
Together with Eckert's and Chiaromonte's labs, we investigate the features determining fragility of the human genome (Fungtammasan et al. 2012). Credit: Daniel Pettruzzi.
...
def slideshow():
	lines = SLIDESHOW.splitlines()
	lines = map(lambda x: x.strip(), lines)
	lines = filter(None, lines)
	group = [iter(lines)] * SLIDESHOW_FIELD_COUNT
	return zip(*group)



PEOPLE_FIELD_COUNT = 6
...
PhD student, Program in Integrative Bioscience - Bioinformatics and Genomics
azf127@psu.edu
My research goal is to differentiate real biological variation from technical errors in Short Tandem Repeats (STRs). I'm interested in mutation pattern of these repeats and how they affect genome and transcriptome stability. I enjoy developing tools (in Python and R) and share them on github and Galaxy.
chai.png
Mc

Wilfried Guiblet
PhD stduent, Program in Integrative Bioscience - Bioinformatics and Genomics
wmg133@psu.edu
...
Monika Michalovova
PhD student, Program in Biology
biomonika@psu.edu
